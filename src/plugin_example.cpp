...
class IdealController : public ControllerBase
{
    public:
        IdealController(EnvironmentBasePtr penv, std::istream& sinput)
            : ControllerBase(penv), cmdid(0), _bPause(false), _bIsDone(true), _bCheckCollision(false), _bThrowExceptions(false), _bEnableLogging(false)
        {
        ...
        virtual bool SetPath(TrajectoryBaseConstPtr ptraj)
        {
            ...
            if( !!ptraj )
            {
                ...
                if( !!_flog && _bEnableLogging )
                {
                    ptraj->serialize(_flog);
                }
